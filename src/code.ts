...
type IsAsync<T> = T extends Prom<infer R> ? true :
	T extends Value<Prom<infer R>> ? true :
	T extends Fun<Prom<infer R>> ? true : false;
...
type Return<T> = IsAsync<T> extends true ? PromiseLike<Unwrap<T>> : Unwrap<T>;
